...
function checksum (str) {
  ...
  return rows.reduce((sum, row) => {
    ...
    const ints = items.map(item => Number(item))
...
function checksum2 (str) {
  ...
  return rows.reduce((sum, row) => {
    ...
    const combinations = kCombinations(ints, 2)

    const evenlyDivisible = combinations.reduce((sum, pair) => {
      const first = pair[0]
      const second = pair[1]
